...
class Mentor(Courses):
    ...
    def add_mentor(self, user_name):
        is_valid_username = DatabaseConnection.get_from_db(get_query.get("GET_FROM_AUTHENTICATION"), (user_name,))
        if is_valid_username:
            user_id = is_valid_username[0][3]
            user_role = DatabaseConnection.get_from_db(get_query.get("GET_USER_ROLES"), (user_id,))
            print(user_role)
            if user_role[0][2] == 3:
