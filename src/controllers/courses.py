...
class Courses:
    ...
    def list_course(self, role, user_id):
        if role == 1:
            content = DatabaseConnection.get_from_db(get_query["GET_COURSES"], ("approved",))
            ...
            content = DatabaseConnection.get_from_db(get_query["GET_COURSES_STATUS"], ("approved", "active"))
            if len(content) == 0:
    ...
    def approve_course(self):
        ...
        if pending_course_count > 0:
            ...
            result = DatabaseConnection.get_from_db(get_query["PENDING_STATUS"], ("pending",))
    ...
    def view_purchased_course(self, user_id):
        content = DatabaseConnection.get_from_db(get_query["GET_STUDENT_COURSES"], (user_id,))
        ...
        for row in content:
            if row[1].lower() == user_input.lower():
                ...
                if len(result) == 0 or result is None:
                    DatabaseConnection.insert_into_db(get_query["INSERT_STUDENT_COURSES"], (user_id, row[0], date.today()))
