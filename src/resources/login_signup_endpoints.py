...
class UserLogin(MethodView):
    def post(self):
        ...
        try:
            is_authenticated = login.login_user(username=user_data["username"], password=user_data["password"])
...
class UserRegister(MethodView):
    def post(self):
        ...
        if validation_response:
            return validation_response, 400
        is_valid = login.sign_up(user_data["name"], user_data["email"], user_data["username"], user_data["password"])
        if is_valid:
            access_token = create_access_token(identity=is_valid)
            return {"access_token": access_token}, 200
        return {"message": "This username already exists."}, 401
